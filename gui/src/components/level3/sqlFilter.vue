...
const getSearchData = (data) => {
	...
	if (!searchParam.value[listItemIndex].value && !searchParam.value[listItemIndex].noValue) {
		if (listItemName == 'comment') {
			showSearchPanel(globalEvent, '', 2, listItemIndex, 'cal')
		} else if (listItemName == 'cal') {
			showSearchPanel(globalEvent, '', 1, listItemIndex, 'value')
		}
	}
...
const getSearchLength=()=>{
	...
	return searchParam.value.length
}
defineExpose({switchFilter, addParam,getSearchLength,showSearchPanel})
</script>
